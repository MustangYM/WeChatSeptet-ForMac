...
- (void)hook_chatLogCellSetTitleLabel:(MMTextField *)arg1
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        arg1.backgroundColor = [YMWeChatConfig sharedConfig].mainChatCellBackgroundColor;
    });
}
...
{
    [self hook_searchCellSetBackgroundColor:[YMWeChatConfig sharedConfig].mainChatCellBackgroundColor];
}
...
- (void)hook_updateNickName
{
    ...
    if ([YMWeChatConfig sharedConfig].usingDarkTheme) {
        [[YMThemeManager shareInstance] changeTheme:cell color:[YMWeChatConfig sharedConfig].mainChatCellBackgroundColor];
