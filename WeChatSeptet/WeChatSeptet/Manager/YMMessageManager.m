...
- (void)asyncRevokeMessage:(MessageData *)revokeMsgData
{
    ...
    if (![[YMWeChatConfig sharedConfig] preventAsyncRevokeSignal] && ![revokeMsgData.fromUsrName containsString:@"@chatroom"]) {
        return;
    }
    
    if (![[YMWeChatConfig sharedConfig] preventAsyncRevokeChatRoom] && [revokeMsgData.fromUsrName containsString:@"@chatroom"]) {
        return;
    }
